...
Scheduler::Scheduler(ifstream& carStream, ifstream& roadStream, ifstream& crossStream, ifstream& presetAnswerStream):
	...
	string line;
	upperBound = (graph.crosses.size() > 135)? 25000 : 18000;
	goCarSize = upperBound/2;
	stride = 50;
	curTime = 0;
...
bool Scheduler::readyToGo(int carIdx) {
	...
	if (priorWay > 50 and not cars[carIdx].prior) {
		return (int)canGoCar.size() < goCarSize*0.9  - priorWay;
	}
