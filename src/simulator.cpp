...
void Scheduler::simulate() {
	...
	while (not taskfinished()) {
		...
		if (not run()) {
			...
			/*
			for (int i = 0; i < (int)graph.roads.size(); i++)
				graph.roads[i].penalty = max(.0, graph.roads[i].penalty -= 0.01);
			 */
		}
		++curTime;
...
void Scheduler::initWaitList() {
	...
	for (int j = 0; j < garageSize; j++) {
		...
		if ((not cars[i].preset or cars[i].reset) and Car::getCarLocation(i) == HOME /*cars[i].startTime == NOT_DECIDED*/ and
			curTime >= cars[i].planTime) {
			if (readyToGo(i)) {
				cars[i].startTime = curTime;
			} else {
				if (not cars[i].preset)
					cars[i].route.clear();
